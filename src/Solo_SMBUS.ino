...
float cell_1_raw = 0; //Value for cell caculations
float cell_2_raw = 0; //Value for cell caculations
float cell_3_raw = 0; //Value for cell caculations
float cell_4_raw = 0; //Value for cell caculations

int pack = 0;   //Pack Voltage
uint16_t packCapacity = 9000, remainingCapacity = 9000; //Define pack total capacity - in mAh
float floatPercentage = 0;
//values for capacity equation
float m = 0;
float c = 0;
uint8_t capacityPercentage = 0;
...
void loop() {
    ...
    if (currentMillisLoop - previousMillisLoop >= intervalLoop) {
        ...
        Cell1Value = analogRead(pinCell_1);
        Cell2Value = analogRead(pinCell_2);
        Cell3Value = analogRead(pinCell_3);
        Cell4Value = analogRead(pinCell_4);
        ApmVValue = analogRead(pinApmV);
        ApmCValue = analogRead(pinCurrent);
        ...
        Serial.println((int) cell_1_raw);
        Serial.print("Cell 1 mV = ");
        Serial.println((int) cell_1_mV);
        Serial.print("Cell 2 read mV = ");
        Serial.println((int) cell_2_raw);
        Serial.print("Cell 3 read mV = ");
        Serial.println((int) cell_3_raw);
        Serial.print("Cell 4 read mV = ");
        Serial.println((int) cell_4_raw);
...
void getVoltageCurrent() {
    ...
    cell_1_raw = cell_temp;
    cell_1_mV = int(
            (float(cell_1_mV) * (1 - voltageFilter))
                    + (cell_temp * voltageFilter));

    //Cell 2
    cell_temp = float(analogRead(pinCell_2));
    //cell_temp = cell_temp *  (vRef/1023);
    cell_temp = cell_temp * ratioCell_2;
    cell_temp = cell_temp - float(cell_1_raw);
    cell_2_raw = cell_temp;
    cell_2_mV = int(
            (float(cell_2_mV) * (1 - voltageFilter))
                    + (cell_temp * voltageFilter));
    ...
    cell_temp = cell_temp - float(cell_1_raw + cell_2_raw);
    cell_3_raw = cell_temp;
    cell_3_mV = int(
            (float(cell_3_mV) * (1 - voltageFilter))
                    + (cell_temp * voltageFilter));
    ...
    cell_temp = cell_temp - float(cell_1_raw + cell_2_raw + cell_3_raw);
    cell_4_raw = cell_temp;
    cell_4_mV = int(
            (float(cell_4_mV) * (1 - voltageFilter))
                    + (cell_temp * voltageFilter));
    ...
    */
    capacityPercentage = constrain(int(floatPercentage), 0, 100);

    remainingCapacity = int( (uint32_t(packCapacity) * capacityPercentage) / 100);
    ...
    */
    current = int32_t(analogRead(A6) * currentRatio );
}
